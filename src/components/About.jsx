            ...
            <div className="rounded-lg p-6 bg-card/50 backdrop-blur-sm border border-primary/20">
              ...
              <div className="grid grid-cols-2 gap-2">
                {personalInfo.skills.map((skill, index) => (
                  <Badge key={index} variant="secondary" className="px-3 py-2 rounded-md bg-primary/10 text-primary border border-primary/20 font-mono text-sm">
                    {skill}
                  </Badge>
                ))}
              </div>
            ...
            <div className="rounded-lg p-6 bg-card/50 backdrop-blur-sm border border-primary/20">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {features.map((feature, index) => (
                  <div key={index} className="p-4 border border-primary/20 rounded-md hover:border-primary transition-colors hover:bg-primary/5">
              ...
              <div className="space-y-6">
                {experience.map((exp, index) => (
                  <div key={exp.id} className="border-l-4 border-primary pl-6 relative">
                    <div className="absolute -left-2 top-0 w-4 h-4 bg-primary rounded-sm"></div>
                    <CardHeader className="p-0 pb-2">
                      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
                        <CardTitle className="text-lg font-mono">{exp.position}</CardTitle>
                        <Badge variant="outline" className="w-fit mt-2 sm:mt-0 rounded-md border-primary/50 font-mono">
                          {exp.period}
                        </Badge>
                      </div>
